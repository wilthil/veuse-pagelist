...
$content .= '<ul class="small-pagelist-block-grid-2 medium-pagelist-block-grid-2 large-pagelist-block-grid-'. $columns .'">';
...
foreach ($featuredpages as $page){
	...
	if( $image == 'true' && has_post_thumbnail($page_id)) {
		...
		if($link == 'true'){
			$content .= '<a href="'. get_permalink($page_id) .'" title="'.$page->page_title.'">';
		}
